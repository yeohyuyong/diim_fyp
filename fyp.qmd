...
time_steps = 751
inoperability_evolution_policy_one = matrix(NA,nrow = num_sectors,ncol=time_steps)
inoperability_evolution_policy_one[,1] = q0

for (t in 2:time_steps) {
  if (t<=55){
    inoperability_evolution_policy_one[,t] = (inoperability_evolution_policy_one[,t-1] + K %*% (A_star %*% inoperability_evolution_policy_one[,t-1] + c_star - inoperability_evolution_policy_one[,t-1]))*0.95
  }
  else { # after 55 days, c* become 0 as no more external shock
    inoperability_evolution_policy_one[,t] = (inoperability_evolution_policy_one[,t-1] + K %*% (A_star %*% inoperability_evolution_policy_one[,t-1] - inoperability_evolution_policy_one[,t-1]))*0.95
  }
}
```
...
```{r}
#| echo: true
# calculating economic loss under policy 1
EL_evolution_policy_one = t(apply(inoperability_evolution_policy_one, 1, cumsum)) * as.vector(x_daily)

total_economic_loss_policy_one = sum(EL_evolution_policy_one[,ncol(EL_evolution_policy_one)])
total_economic_loss_policy_one
```
...
```{r}
#| echo: true
inoperability_evolution_policy_two = matrix(NA,nrow = num_sectors,ncol=time_steps)
inoperability_evolution_policy_two[,1] = q0

for (t in 2:time_steps) {
  if (t<=55-20){
    inoperability_evolution_policy_two[,t] = (inoperability_evolution_policy_two[,t-1] + K %*% (A_star %*% inoperability_evolution_policy_two[,t-1] + c_star - inoperability_evolution_policy_two[,t-1]))
  }
  else { # after 55 days, c* become 0 as no more external shock
    inoperability_evolution_policy_two[,t] = (inoperability_evolution_policy_two[,t-1] + K %*% (A_star %*% inoperability_evolution_policy_two[,t-1] - inoperability_evolution_policy_two[,t-1]))
  }
}
```
...
```{r}
#| echo: true
# calculating economic loss under policy 2
EL_evolution_policy_two = t(apply(inoperability_evolution_policy_two, 1, cumsum)) * as.vector(x_daily)

total_economic_loss_policy_two = sum(EL_evolution_policy_two[,ncol(EL_evolution_policy_two)])
total_economic_loss_policy_two
```
...
print("\n=== SUMMARY STATISTICS BY LOCKDOWN DURATION ===\n")
print(summary_stats)
```
